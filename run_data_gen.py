...
if __name__ == "__main__": 
    ...
    # Load dict with all recorded scenes
    with open(os.path.join('./data', 'scene_dict.json'), 'r', encoding='utf-8') as f:
        scene_dict = json.load(f)
    

    for bag in os.listdir('./data/rosbags'):
        bag_name = bag
        scene_id, scene_split = bag.split(".")[0].split("_")
        si = str(int(scene_id))
        class_id = scene_dict[si]["class_id"]
        obj_offset = scene_dict[si]["world_offset"]
        box_corners = scene_dict[si]["box_corners"]
        root_dir = "./"
